...
EVALUATE="${PROMPTS_DIR}/evaluate.md"
...
stage_evaluate() {
  ...
  # Quality degrades past 3 cycles; if it's not fixed by then,
  # the approach needs to change, not iterate further.
...
stage_evaluate_check() {
  local task_id="$1" tier="$2"
  local task_dir="${FORGE_DIR}/pipeline/runs/${task_id}"

  # Find the latest evaluation file
  local latest_iteration=1
  while [[ -f "${task_dir}/evaluation-$((latest_iteration + 1)).json" ]]; do
    latest_iteration=$((latest_iteration + 1))
  done

  local eval_file="${task_dir}/evaluation-${latest_iteration}.json"
  if [[ ! -f "$eval_file" ]]; then
    emit_error "evaluate-check" "No evaluation file found" "" ""
    exit 1
  fi

  local verdict
  verdict=$(jq -r '.verdict // "fail"' "$eval_file")

  case "$verdict" in
    pass)
      # Evaluation passed — proceed to delivery
      echo "pass"
      ;;
    revise)
      # Needs revision — go back to execute with revision brief
      echo "revise"
      ;;
    fail)
      emit_error "evaluate" "Evaluation failed" "$eval_file" \
        "bash ${PIPELINE_DIR}/orchestrator.sh --resume ${task_id} --stage execute"
      exit 1
      ;;
    *)
      emit_error "evaluate-check" "Unknown verdict: ${verdict}" "$eval_file" ""
      exit 1
      ;;
  esac
}
...
stage_verify() {
  ...
  # Mechanical checks passed — now run tri-agent evaluation
  stage_evaluate "$task_id" "$tier"
}
...
if [[ -n "$RESUME_ID" && -n "$RESUME_STAGE" ]]; then
  ...
  case "$RESUME_STAGE" in
    ...
    evaluate)
      ...
      ;;
    evaluate-check)
      eval_result=$(stage_evaluate_check "$task_id" "$tier")
      if [[ "$eval_result" == "revise" ]]; then
